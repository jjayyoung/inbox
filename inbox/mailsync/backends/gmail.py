...
class GmailFolderSyncEngine(CondstoreFolderSyncEngine):
    ...
    def initial_sync_impl(self, crispin_client, local_uids,
        ...
        remote_g_metadata, update_uid_count = self.fetch_g_metadata(
                ...
                if uid in flags:
                    message_download_stack.put(
                        GMessage(uid, remote_g_metadata[uid], flags[uid].flags,
                                 flags[uid].labels))
            new_uid_poller = spawn(self.check_new_g_thrids,
                                   message_download_stack)
            self.download_queued_threads(crispin_client,
                                         message_download_stack)
        elif self.folder_name in uid_download_folders(crispin_client):
            full_download = self.deduplicate_message_download(
    ...
    def highestmodseq_callback(self, crispin_client, new_uids, updated_uids):
        ...
        to_download = self.deduplicate_message_download(
    ...
    def fetch_g_metadata(self, crispin_client, uids):
        ...
        if saved_highestmodseq is not None:
            ...
            remote_g_metadata, update_uid_count = \
                self.retrieve_saved_g_metadata(crispin_client, uids,
                                               saved_highestmodseq)
    ...
    def check_new_g_thrids(self, message_download_stack):
        """
    ...
    def deduplicate_message_download(self, crispin_client, remote_g_metadata,
                                     uids):
        """
    ...
    def deduplicate_message_object_creation(self, db_session, raw_messages):
    ...
    def create_message(self, db_session, acct, folder, msg):
        ...
        new_uid = common.create_imap_message(db_session, log, acct, folder,
                                             msg)
        new_uid = self.add_gmail_attrs(db_session, new_uid, msg.flags, folder,
                                       msg.g_thrid, msg.g_msgid, msg.g_labels,
                                       msg.created)
        return new_uid

    def add_gmail_attrs(self, db_session, new_uid, flags, folder, g_thrid,
    ...
    def download_and_commit_uids(self, crispin_client, folder_name, uids):
        ...
        with self.syncmanager_lock:
            ...
            # lock
            with mailsync_session_scope() as db_session:
                raw_messages = self.deduplicate_message_object_creation(
    ...
    def download_queued_threads(self, crispin_client, message_download_stack):
        """
        ...
        with connection_pool(self.account_id).get() as all_mail_crispin_client:
            ...
            while not message_download_stack.empty():
                ...
                thread_g_metadata = all_mail_crispin_client.g_metadata(
                    thread_uids)
                self.download_thread(all_mail_crispin_client,
                                     thread_g_metadata,
    ...
    def download_thread(self, crispin_client, thread_g_metadata, g_thrid,
                        thread_uids):
        """
    ...
    def retrieve_saved_g_metadata(self, crispin_client, local_uids,
                                  saved_highestmodseq):
        ...
        if remote_g_metadata is not None:
            ...
            if crispin_client.selected_highestmodseq > saved_highestmodseq:
                update_uid_count = self.update_saved_g_metadata(
                    crispin_client, remote_g_metadata, local_uids)
        else:
            log.info("No cached data found")
        return remote_g_metadata, update_uid_count

    def update_saved_g_metadata(self, crispin_client, remote_g_metadata,
                                local_uids):
        """
